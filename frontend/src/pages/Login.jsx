...
const Login = () => {
  ...
  const {dispatch}=useContext(AuthContext) //destructuring
  ...
  const submitHandler=async event=>{
    ...
    try {
     ...
     //after succesful login we dispatch the action and pass the necesary info  as a payload
    ...
    <section className='px-5 lg:px-0'>
      <div className='w-full max-w-[570px] mx-auto rounded-lg shadow-md md:p-10'>
        ...
        <form className='py-4 md:py-0' onSubmit={submitHandler} >
          ...
          <div className='m-7'>
            <button type='submit' className='w-full bg-primaryColor text-white text-[18px] leading-[38px] rounded-lg px-4 py-3'>{ loading?<HashLoader size={25} color='#fff'/>:'Login'} 
            </button>
          </div>
